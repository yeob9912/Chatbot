...
export default function SignupPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            let data = await response.json().catch(() => ({ msg: 'Invalid server response' }));

            // If user already exists, try logging them in instead
            if (!response.ok && data.msg === 'User already exists') {
                response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/login`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ email, password })
                });
                data = await response.json().catch(() => ({ msg: 'Invalid server response' }));
            }
            ...
            setError(err.message === 'Failed to fetch' ? 'Connection to server failed ' : `Error: ${err.message}`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleOAuthLogin = (provider: string) => {
        window.location.href = `${process.env.NEXT_PUBLIC_API_URL}/api/auth/${provider}?mode=signup`;
    };
    ...
    return (
        <div className="auth-page">
            <div className="auth-card glass">
                ...
                <p className="auth-subtitle">Join ASTU Helper bot</p>

                {error && !error.includes('Password') && <div className="auth-error-banner">{error}</div>}
                ...
                <form onSubmit={handleSubmit} className="auth-form">
                    ...
                    <div className="form-group">
                        ...
                        <input
                            ...
                            required
                            style={error.includes('Password') ? { borderColor: '#ff4d4d' } : {}}
                        />
